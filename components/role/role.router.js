...
router.get(`/roles`, async (req, res, next) => {
  ...
  return tryExecute(async _=> res.locals.result = {data: await rc.getRoles(req.query)}, next);
});
...
router.get(`/role/:roleId`, async (req, res, next) => {
  ...
  return tryExecute(async _=> res.locals.result = {data: await rc.getRole(req.params)}, next);
});
